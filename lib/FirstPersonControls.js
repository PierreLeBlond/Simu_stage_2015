 ...
 *
 * adapted from THREE.FirstPersonControls by 
 *
 ...
 *
 * move: z/q/s/d or up/down/left/right
 * rotate: left mouse
 * pan: right mouse
...
THREE.FirstPersonControls = function ( object, domElement ) {
	...
	this.update = function (delta) {
		//this.object.rotation.order = 'ZYX'; -> Was put in the main index script
	...
	function onMouseMove( event ) {
		...
		if ( state === STATE.ROTATE ) {
			...
			panDelta.multiplyScalar(0.5).multiplyScalar(scope.moveSpeed);
	...
	function onMouseWheel(event) {
		...
		scope.moveSpeed = Math.max(0, scope.moveSpeed);
		scope.moveSpeed = Math.min(5, scope.moveSpeed);
		scope.dispatchEvent( startEvent );
		scope.dispatchEvent( endEvent );
	}
	...
	function onKeyDown( event ) {
		...
		switch ( event.keyCode ) {
			...
			case scope.keys.Z: scope.moveForward = true; break;
			case scope.keys.S: scope.moveBackward = true; break;
			case scope.keys.Q: scope.moveLeft = true; break;
	...
	function onKeyUp( event ) {
		switch ( event.keyCode ) {
			case scope.keys.Z: scope.moveForward = false; break;
			case scope.keys.S: scope.moveBackward = false; break;
			case scope.keys.Q: scope.moveLeft = false; break;
