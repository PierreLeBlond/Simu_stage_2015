 ...
 *
 * adapted from THREE.EarthControls by 
 *
...
THREE.EarthControls = function ( camera, renderer, scene ) {
		...
		if(pivot){
			if(state === STATE.DRAG){
				...
				if(diff.y > 0){
					...
					amount = diff.y*2 - Math.max(0, this.minAngle - (angle - diff.y));
				}else{
					// rotate upwards and apply maxAngle limit
					amount = diff.y*2 + Math.max(0, (angle - diff.y) - this.maxAngle);
				}
				p.rotateOnAxis(side, -amount);*/

				p.rotateOnAxis(side, -diff.y*5);
	...
	function onMouseDown( event ) {
		...
		var I = getMousePointCloudIntersection(mouse);
		I = (I) ? I.point : null;
